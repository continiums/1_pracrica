...
void draw(struct record* records, int iod)
{
...
int main()
{
	...
	draw(records);
	cout << " (вариант 9) \nУдалить все записи (элементы массива структур), содержащее введенное значение даты" << endl << endl << endl;

	int day, month, year;
	cout << "введите день, месяц, год строки, которую предстоит удалить \n";
a1:	cin >> day >> month >> year ;
	cout << "выбранная дата:\t" << day << "." << month << "." << year << endl;
	int iod = -1;
	for (int  i = 0; i < 3; i++)
	{
		if ((records[i].Date.day == day) and (records[i].Date.month == month) and (records[i].Date.year == year)) iod = i;
	}
	cout << "удаляется строка №" << iod+1 << endl;
	if (iod == -1)
	{
